...
export async function serialize(env: Environment<DataFactory | FormatsFactory | TermMapFactory>, dataset: DatasetCore, { renameBlankNodes, format, prefixes = [] }: SerializeArgs): Promise<string> {
  ...
  let stream: Readable = toStream(dataset) as unknown as Readable
  if (renameBlankNodes) {
    stream = stream.pipe(new RenameBlankNodes(env) as unknown as NodeJS.WritableStream) as unknown as Readable
  }
...
class RenameBlankNodes extends Transform {
  ...
  override _transform(chunk: Quad, encoding: string, callback: (error?: Error | null, data?: Quad) => void): void {
    let replaced = false
